...
foreach (@map){
	print qq!<area shape="rect" coords="$_->[0],0,$_->[1],420" href="/cgi-bin/HM/ShowRegion.cgi?species=$HM->{species}&analysis_no=$analysis_no&chromosome=$_->[2]!.($unique_id?'&unique_id='.$unique_id:'').qq!">\n!;
}
print "</map>\n";
my $table_start=470;
if ($FORM{chromosome}){
...
if (@regions){
	...
	print qq °
		...
		<FORM action="/cgi-bin/HM/CallGeneDistillerWithAllRegions.cgi" method="post" name="GeneDistiller" target="_blank">
		<TABLE cellpadding="5" style="border: 1px solid #0000FF;" width="100%">
		<TR><TD><i class="bold" style="border-bottom: solid 1px #999999; font-size: 12pt; color:#555555">call GeneDistiller</i></td>
		<td>
		<input type="hidden" name="analysis_no" value="$analysis_no">
		<input type="hidden" name="species" value="$HM->{species}">
		<input type="hidden" name="unique_id" value="$unique_id">
	...
	print qq °
	<FORM action="/cgi-bin/HM/CreateAlohomoraFiles.cgi" method="post" name="alohomora" target="_blank">
	<input type="hidden" name="analysis_no" value="$analysis_no">
	<input type="hidden" name="species" value="$HM->{species}">
	<input type="hidden" name="unique_id" value="$unique_id">
	<TABLE cellpadding="5" style="border: 1px solid #0000FF;" width="100%">
	...
	<input type="submit" name="submit" value="create files"></td></tr></table>
		...
		<FORM action="/cgi-bin/CreateBEDfile.cgi" method="post" name="BEDfile" target="_blank">
				<input type="hidden" name="many_regions" value="1">
		<input type="hidden" name="type" value="region">
		<TABLE cellpadding="5" style="border: 1px solid #0000FF;" width="100%">
		<tr><td colspan="4">
		<i class="bold"  style="border-bottom: solid 1px #999999; font-size: 12pt; color:#555555">create a BED file</i></td></tr>
		<TR><TD>
