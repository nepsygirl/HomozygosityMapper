...
my $analysesref=$HM->AllAnalyses($user_id,undef,$unique_id);
...
#	aallele_frequencies,  max_block_length, user_login, marker_count, vcf_build, homogeneity_required	lower_limit, date	
		@FORM {qw /project_no analysis_name analysis_description
	allele_frequencies limit_block_length homogeneity_required lower_limit/}=@{$_}[0,3,4,5,6,10,11];
		die ("$_->[6]: ".$FORM{limit_block_length }."\n".join (",",%FORM));
...
foreach (@$projectsref){
	...
	my $selected=($_->[0]==$FORM{project_no}?'selected':'');
...
foreach my $item (qw /cases_ids controls_ids analysis_name analysis_description
		 limit_block_length lower_limit/){
...
#	die if $FORM{$item};
	$template->param($item => 'checked') if $FORM{$item};	
}
