...
close (STDIN);
if ($^O=~/win32/i){
	exec ("c:/bin/perl/bin/perl d:/www/cgi-bin/HM/AnalyseVCF.pl \"".join ('" "',map {"$_=$FORM{$_}"} keys %FORM).'"');
}
else {
	unless (open F, "-|") {
		open STDERR, ">&=1";
		exec ("/usr/bin/perl /www/cgi-bin/HM/AnalyseVCF.pl \"".join ('" "',map {"$_=$FORM{$_}"} keys %FORM).'"');
