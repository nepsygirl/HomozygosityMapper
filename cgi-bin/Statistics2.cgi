...
my $filename='HM_statistics2_'.$date;
...
foreach my $species ('human','cow','dog','horse','mouse','rat','sheep'){
	...
	print qq ! <tr><td colspan="4"><b>$species</b></td></TR> \n !;
	...
	#	print "<tr><td colspan='2'>$project_no,$markers_on_chip,$genotypes,$deleted,$archived,$isvcf</td></tr>\n ";
		unless ($deleted or $archived or $genotypes) {
			my $sth2=$dbh->prepare ("SELECT COUNT(*) FROM ".$HM->{data_prefix}.($isvcf?'vcf':'')."genotypes_".$project->[0])  || $HM->PegOut(" -> $HM->{data_prefix} <- $DBI::errstr");
...
#			print "<pre>$markers_on_chip</pre>";
	...
	$sth->execute ()  || $HM->PegOut($DBI::errstr);
	print qq !	<tr><td>&nbsp;</td><td colspan="2">analyses</td>
		<td style="text-align: right">!,commify($sth->fetchrow_arrayref->[0]),"</td></tr>\n";		
}	
print "</table>";
print qq !<A HREF="http://www.homozygositymapper.org/index.html">back to the homepage</A> !;
$HM->EndOutput();
