...
$HM->StartOutput ("Alohomora files for linkage analysis");
...
sub Zip {
	...
	system "cp /www/HomozygosityMapper/doc/HomozygosityMapper_to_Alohomora.txt /tmp/";
	my $command="zip ".$zipfile.".zip ".$zipfile.".map ".$zipfile.".freq ".$zipfile.".genotypes  HomozygosityMapper_to_Alohomora.txt >> /dev/null";
	chdir "/tmp/";
	system ($command);
	print '<h2><A HREF="/temp/',$zipfile,'.zip">download files for Alohomora</A></h2>
	<hr><h3>Instructions</h3>
	These instructions are <b>not</b> ';
	print qq !<A HREF="http://gmc.mdc-berlin.de/alohomora/docs.zip" target="_blank">Alohomora's manual</A>. 
	Please read it if you are not familiar with
	the program. You can download Alohomora for various operating systems 
	<A HREF="http://gmc.mdc-berlin.de/alohomora/" target="_blank">here</A>.
	<ul>
	<li>download the zip file</li>
	<li>create a folder for your analysis</li>
	<li>unzip the zip file into this folder</li>
	<li>don't forget create a pedfile in this folder (or copy an existing one)</li>
	<li>start Alohomora</li>
	<li>set the 'Other' map file to the downloaded *.map file ('Options' -> 'File selection')</li>
	<li>set the 'Other' allele frequencies file to the downloaded *.freq file ('Options' -> 'File selection')</li>
	<li>set the 'Genetic map' to 'Other' (left panel)</li>
	<li>set the 'Allele frequencies' to 'Other' (left panel)</li>
	<li>select the *.genotypes file as 'Genotypefile' (line below the drop-down menus)</li>
	<li>run PedCheck (below the drop-down menus) to remove Mendelian inconsistencies</li>
	<li>choose the linkage program of choice ('DataConversion')<br>
	(and don't forget to choose a recessive 'Genetic Model' [f1: 0, f2: 0, f3: 1])</li>
	</ul>
	</BODY></HTML>\n !; # " '
	unlink "/tmp/".$zipfile.".map","/tmp/".$zipfile.".freq","/tmp/".$zipfile.".genotypes",'HomozygosityMapper_to_Alohomora.txt';
}

sub Query {
	my ($reg,$id,$freqs,$zipfile)=@_;
	my @regions=split /,/,$reg;
	print "Creating files...<br>\n";
...
#	print "$sql<br>\n";
	...
	for my $i (0..$#$results){
		unless ($i%5000){
			print "$i / ".(scalar @$results)." genotypes...<br>\n";
		}
...
#	print "222 / ".(scalar @$results)." genotypes...<br>\n";
