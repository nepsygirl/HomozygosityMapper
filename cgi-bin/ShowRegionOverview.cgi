#!/usr/bin/perl
#http://www.homozygositymapper.org/HM/ShowRegion2.cgi?project_no=203455&analysis_no=56271
$|=1;
...
#print Dumper($HM);
my $marker_count=$HM->{marker_count};
...
my $out=(join (",",%FORM));
if ($FORM{chromosome}){
	$HM->StartOutput(qq *Overview of homozygosity on chromosome $FORM{chromosome} in $HM->{project_name} - <i class="blue">$HM->{analysis_name}</i>*);
}
else {
	$HM->StartOutput(qq *Overview of genome-wide homozygosity in $HM->{project_name} - <i class="blue">$HM->{analysis_name}</i>*);
}
print qq!<small class="red">too many markers ($marker_count) - we're switching to an overview...</small>\n!;
...
my $sql="";
if ($HM->{vcf}){
	$sql="SELECT chromosome, round(position/10000)*10000  , MAX(score),round(position/10000)*10000 FROM
...
$sql.=" GROUP BY chromosome, round(position/10000)  
ORDER BY chromosome, round(position/10000) ";
...
foreach my $i (0..$#$results){
	my ($chromosome, $position, $score, $marker_no)=@{$results->[$i]};
...
print "</map>\n";
print qq!<small class="red">Too many markers ($marker_count) - this is only an overview.<br>Please note that consecutive markers were aggregated and only the highest score is shown - the regions are hence not accurate. Click on a chromosome to zoom in.</small>\n!;
my $table_start=470;
if ($FORM{chromosome}){
	$table_start+=100;
...
print qq !<FORM action="/HM/ShowRegion.cgi" method="post" name="setthreshold" >\n!;
foreach my $key (keys %FORM){
...
print qq!</SELECT><INPUT TYPE="submit" value="change"></tr></table></FORM><br>\n!;
# print $out,"<br>" if $analysis_no==85;
# print join (",",%FORM),"<br>" if $analysis_no==85;
if (@regions) {
	...
	print "</table><br>";
	print qq !<A HREF="$b36_link" class="red">new &amp; highly experimental: show this for build 36</A>! if $FORM{species} eq 'human';
print "</DIV></DIV>";
$HM->EndOutput();
...
sub DisplayTableWithLinks {
	...
	foreach (sort {$b->[0] <=> $a->[0]}@$aref){
		...
		my $link_region="/HM/ShowRegion.cgi?species=$HM->{species}&chromosome=".$chromosome.'&start_pos='.$start_pos.'&end_pos='.$end_pos.'&analysis_no='.$analysis_no.'&margin='.$margin;
		$link_region='&build='.$FORM{build} if $FORM{build}==36;
		my $link_genotypes="/HM/DisplayGenotypes.cgi?species=$HM->{species}&chromosome=".$chromosome.'&start_pos='.$start_pos.'&end_pos='.$end_pos.'&start_snp='.$start_snp.'&end_snp='.$end_snp.'&analysis_no='.$analysis_no.'&margin='.$margin;
