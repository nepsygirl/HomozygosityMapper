...
use Sort::Naturally;
use lib '/www/lib';
my $baseurl='/HM/DisplayGenotypes.cgi?';
...
my $margin=(length $FORM{margin}? $FORM{margin}: $HM->SetMargin);

my $is_vcf=$HM->{vcf};
my $snp_prefix='rs';
if ($HM->{species} && $HM->{species} ne 'human') {
	$snp_prefix='#';
}
elsif ($is_vcf) {
		$snp_prefix='';
...
#	$HM->PegOut("Please use either SNP ID <u>or</u> position to indicate the region's start.")
#		if length $FORM{start_pos} && $FORM{start_snp};
#	$HM->PegOut("Please use either SNP ID <u>or</u> the position to indicate the region's end.")
#		if length $FORM{end_pos} && $FORM{end_snp};
	my $snp_pos=$HM->{dbh}->prepare ("SELECT position FROM $HM->{markers_table} WHERE dbsnp_no=? AND chromosome=?") || $HM->PegOut($DBI::errstr);
	if ($FORM{start_snp} && ! $FORM{start_pos}){
		...
		$HM->PegOut("SNP $snp_prefix$FORM{start_snp} was not found") unless ref $pos eq 'ARRAY' and $FORM{start_pos}=$pos->[0];
	}
	if ($FORM{end_snp}&& ! $FORM{end_pos}){
		$FORM{end_snp}=~s/^rs//i;
		$snp_pos->execute($FORM{end_snp},$FORM{chromosome})  || $HM->PegOut($DBI::errstr);
		my $pos=$snp_pos->fetchrow_arrayref;
		$HM->PegOut("SNP $snp_prefix$FORM{end_snp} was not found") unless ref $pos eq 'ARRAY' and $FORM{end_pos}=$pos->[0];
	}
}

$HM->StartOutput( ($HM->{species} ne 'human'?$HM->{species}.': ':'') .
	qq !$HM->{project_name} - <i class="blue">$HM->{analysis_name}</i>!);
...
if ($is_vcf){
	$sql="SELECT r.position,r.position,sample_id,genotype,block_length,score, hom_freq, hom_freq_ref, affected FROM
		$HM->{data_prefix}samples_".$HM->{project_no}." s, $HM->{data_prefix}vcfgenotypes_".$HM->{project_no}." gt,   $HM->{data_prefix}samples_".$id." sa, $HM->{data_prefix}vcfresults_".$id." r
	...
	$sql="SELECT m.dbsnp_no,position,sample_id,genotype,block_length,score, hom_freq, hom_freq_ref, affected FROM
		$HM->{data_prefix}samples_".$HM->{project_no}." s, $HM->{data_prefix}genotypes_".$HM->{project_no}." gt, $HM->{markers_table} m,  $HM->{data_prefix}samples_".$id." sa, $HM->{data_prefix}results_".$id." r
		WHERE s.sample_no=gt.sample_no
		AND sa.sample_no=s.sample_no
...
if ($is_vcf){
	$query_gt->execute($FORM{chromosome},$FORM{chromosome},$FORM{start_pos}-$margin,$FORM{end_pos}+$margin) || $HM->PegOut({text=>[($sql,$DBI::errstr)]});
}
else {
...
<script src="/javascript/HomozygosityMapper_DefineIntervalForGeneDistiller_VCF.js" type="text/javascript">
</script>
^;
print "<small>",scalar @$results," genotypes...</small>\n";
...
print qq !<FORM name="form1" action="/GD/API.cgi" method="POST">
<INPUT TYPE="hidden" name="x1" value="$LINK{x1}" size="10">
<INPUT TYPE="hidden" name="x2" value="$LINK{x2}" size="10">
<INPUT TYPE="hidden" name="vcfbuild" value="$is_vcf" size="10">
...
<td>
	...
	print qq !disabled="disabled"! if $HM->{species} ne 'human';  # "
	print "></td>\n";
...
my $include_vcf='var vcfbuild='.($is_vcf||0).';';
print qq !	<br><DIV id="linkself"><A HREF="$link">Right-click to bookmark this output.</A><br>
	<A HREF="$link3">Click here for the genotypes table.</A></DIV> !;
unless ($size==7) {
	print qq !<br><b class="red">Original image was too large to be displayed - this is a compression. This feature is still in beta stage - please report any errors/annoyances.</b>!;
}
print qq !	</td>
	<td><A HREF="/documentation.html#genotypesview" TARGET="doc">help</A>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<A HREF="/tutorial.html#genotypesview" TARGET="doc">tutorial</A>
</td></tr>
	</table>
</FORM>\n</DIV>
<DIV id="info" style="border: 0px none rgb(0, 0, 0); overflow: visible; position: absolute; top: !.$div_y_start.qq!px;
visibility: visible; z-index: 20; left: 100px; "> !; #"
print	"$start_snp - $end_snp" unless $is_vcf;
print qq !<br>$FORM{start_pos} - $FORM{end_pos} bp
</DIV>
...
if ($user_agent=~/MSIE/ && $snps>600){
	print qq !<b class="red">You seem to be using Microsoft Internet Explorer (!.$ENV{HTTP_USER_AGENT}.qq !). IE is known to have problems displaying
