...
foreach my $i (0..$#$results){
	...
	if ($freq_hom_ref){
		...
		if ($surplus<0){
			$image2->line($x,20,$x,20+$freq_hom_ref*20/1000,$colour{black});
			$image2->line($x,20+$freq_hom_ref*20/1000,$x,20+$freq_hom*20/1000,$colour{blue});
		}
		elsif  ($surplus>0){
			$image2->line($x,20,$x,20-$freq_hom_ref*20/1000,$colour{black});
			$image2->line($x,20-$freq_hom_ref*20/1000,$x,20-$freq_hom*20/1000,$colour{red}) ;
		}
	}
	if ($score>=$threshold){
...
my $link="http://www.homozygositymapper.org/HM/PrintScores.cgi?project_no=$project_no&analysis_no=$analysis_no";
...
foreach my $thresh ('0.99','0.9','0.8','0.7','0.6','0.5'){
...
if (@regions) {
	...
	print "</table><br>";
	print qq !<BR><A HREF="$link">retrieve homozyosity scores</A><BR>\n!;
...
sub DisplayTableWithLinks {
	...
	foreach (sort {$b->[0] <=> $a->[0]}@$aref){
		...
		$start_snp=($HM->{vcf} ?'n/a':$snp_prefix.$start_snp);
		$end_snp=($HM->{vcf} ?'n/a':$snp_prefix.$end_snp);
		print qq !<TR><td>$score</td><td>!,join ("</td><td>",$chromosome,$start_pos,$end_pos,$start_snp,$end_snp),
