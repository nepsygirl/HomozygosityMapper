...
my $link="http://www.homozygositymapper.org/HM/ShowRegion.cgi?analysis_no=$analysis_no&species=$HM->{species}";
if ($FORM{unique_id}) {
	...
	Delete data: <br>
	<A HREF="http://www.homozygositymapper.org/HM/SelectDelete.cgi?unique_id=$FORM{unique_id}">http://www.homozygositymapper.org/HM/SelectDelete.cgi?unique_id=$FORM{unique_id}</A><br>!;
}
